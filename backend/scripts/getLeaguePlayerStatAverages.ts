...
async function getLeaguePlayerStatAveragesAll(
  season: string,
  seasonType: string,
) {
  ...
  const savedResult = await prisma.playerLeagueAverages.upsert({
    where: {
      season_position_unique: {
        season,
        seasonType,
        position: 'all',
      },
...
async function getLeaguePlayerStatAveragesByPosition(
  season: string,
  seasonType: string,
  position: string,
) {
...
if (!season || !seasonType) {
  console.error(
    'Usage: ts-node getLeaguePlayerStatAverages.ts <season> <seasonType>',
  );
  process.exit(1);
}
...
getLeaguePlayerStatAveragesAll(season, seasonType)
  .then(() => {
    const positions = ['G', 'G-F', 'F', 'F-C', 'C', 'C-F', 'F-G'];
    return Promise.all(
      positions.map((position) =>
        getLeaguePlayerStatAveragesByPosition(season, seasonType, position),
      ),
    );
  })
  .catch((error) => {
